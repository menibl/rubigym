                ...
                {draft.exercises.filter(exercise => (exercise.dayNumber || 1) === selectedDraftDay).map((exercise, index) => (
                  <article key={exercise.id} className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                    ...
                    <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                      ...
                      <label className="text-[9px] font-bold text-slate-500">ציוד / מכונה<input value={exercise.equipment || ''} onChange={event => updateExercise(exercise.id, { equipment: event.target.value })} className="mt-0.5 w-full rounded border border-slate-200 bg-white px-2 py-1 text-xs text-slate-800" placeholder="לדוגמה: מכונת לחיצת רגליים" /></label>
