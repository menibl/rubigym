                      ...
                      <div className="space-y-2">
                        {station.exercises.map((exercise, exerciseIndex) => (
                          <div key={exercise.id} className="rounded-xl border border-slate-200 bg-white p-3">
                            ...
                            <div className="mt-2 grid grid-cols-2 gap-2"><input value={exercise.weight || exercise.reps} onChange={event => updateStationExercise(station, exercise.id, { weight: event.target.value })} className="rounded-lg border border-slate-200 px-2 py-1.5 text-[10px]" placeholder="חזרות / משקל" /><input value={exercise.equipment || ''} onChange={event => updateStationExercise(station, exercise.id, { equipment: event.target.value })} className="rounded-lg border border-slate-200 px-2 py-1.5 text-[10px]" placeholder="ציוד / מכונה נדרשים" /><input value={exercise.notes || ''} onChange={event => updateStationExercise(station, exercise.id, { notes: event.target.value })} className="col-span-2 rounded-lg border border-slate-200 px-2 py-1.5 text-[10px]" placeholder="דגש למאמן" /></div>
                  ...
                  <article key={exercise.id} className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                    <div className="flex flex-col gap-3 lg:flex-row lg:items-start">
                      ...
                      <div className="grid min-w-0 flex-1 gap-2 sm:grid-cols-2 lg:grid-cols-6">
                        ...
                        <label className="sm:col-span-2 lg:col-span-2 text-[9px] font-bold text-slate-500">ציוד / מכונה<input value={exercise.equipment || ''} onChange={event => updateExercise(exercise.id, { equipment: event.target.value })} className="mt-1 w-full rounded-lg border border-slate-300 bg-white px-2 py-2 text-xs" placeholder="לדוגמה: ארגז ומדרגה" /></label>
                        <label className="sm:col-span-2 lg:col-span-3 text-[9px] font-bold text-slate-500">הנחיות למאמן<input value={exercise.notes || ''} onChange={event => updateExercise(exercise.id, { notes: event.target.value })} className="mt-1 w-full rounded-lg border border-slate-300 bg-white px-2 py-2 text-xs" placeholder="טכניקה והתאמות" /></label>
