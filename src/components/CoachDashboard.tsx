...
import {
  ...
  createPersonalLibraryEntry,
  isActiveNutritionPlan,
  isActivePersonalPlan,
  isNutritionLibraryEntry,
  isPersonalLibraryEntry,
  nutritionLibraryTitle,
  personalLibraryTitle
} from '../data/programLibrary';
import {
  BookOpen,
  Apple,
  MessageSquare,
...
export const CoachDashboard: React.FC<CoachDashboardProps> = ({
  ...
  const selectedTrainee: User | undefined = registeredTrainee || (isGeneralPersonalProgram && generalPersonalTarget ? {
    id: generalPersonalTarget.id,
    name: generalPersonalTarget.name,
    email: '',
    phone: '',
  ...
  const currentNutrition = nutritionPlans.find(np => np.traineeId === selectedTraineeId && isActiveNutritionPlan(np));
  ...
  const handleSaveNutrition = (e?: React.FormEvent, override?: Partial<typeof nutritionForm>) => {
    ...
    if (currentNutrition) {
      activePlan = {
        ...currentNutrition,
        title: nutritionLibraryTitle(selectedTrainee.name, createdAt),
        sourcePlanId: sourcePlanId || currentNutrition.sourcePlanId,
        libraryEntry: false,
        active: true,
  ...
  const handleAddExerciseToPlan = async (e: React.FormEvent) => {
    ...
    const currentPlan = workoutPlans.find(wp => wp.traineeId === selectedTraineeId && isActivePersonalPlan(wp));
  ...
  const handleDeleteExercise = (exerciseId: string) => {
    const exercise = traineeWorkoutPlan?.exercises.find(item => item.id === exerciseId);
    ...
    const updatedPlans = workoutPlans.map(wp => {
      if (wp.id === traineeWorkoutPlan?.id) {
        return {
          ...wp,
  ...
  const traineeWorkoutPlan = workoutPlans.find(wp => wp.traineeId === selectedTraineeId && isActivePersonalPlan(wp));
  ...
  const personalSetupQuestions: WizardQuestion[] = isGeneralPersonalProgram ? [
    ...
    { id: 'experienceLevel', label: 'לאיזו רמת ניסיון התוכנית מיועדת?', type: 'choice', required: true, options: [{ value: 'BEGINNER', label: 'מתחילים' }, { value: 'INTERMEDIATE', label: 'בינוניים' }, { value: 'ADVANCED', label: 'מתקדמים' }] },
  ...
  const completePersonalSetup = (answers: WizardAnswers) => {
    if (!selectedTrainee) return;
    const now = new Date().toISOString();
    const generalProgramName = String(answers.programName || generalPersonalTarget?.name || '').trim();
    if (isGeneralPersonalProgram && generalPersonalTarget && generalProgramName && generalProgramName !== generalPersonalTarget.name) {
      setGeneralPersonalTarget({ ...generalPersonalTarget, name: generalProgramName });
    }
    if (!isGeneralPersonalProgram && answers.updateProfile === true) {
    ...
    handleUpdateAssistantDraft({
      ...
      objective: isGeneralPersonalProgram
        ? generalProgramName || baseDraft.objective
    ...
    const context = isGeneralPersonalProgram
      ? `נתוני התוכנית האישית הכללית „${generalProgramName}” נקלטו. התוכנית אינה משויכת למתאמן ותישמר במאגר לשימוש עתידי. אם חסר מידע מהותי, אשאל שאלה קצרה אחת בכל פעם. כתוב “בנה תוכנית אימון” כדי להתחיל.`
      : `נתוני הפתיחה נקלטו: ${answers.updateProfile === true ? 'נתוני המתאמן עודכנו' : 'שימוש בנתונים הקיימים'}; ${source ? 'נטענה תוכנית מהמאגר כטיוטה' : 'נבנית תוכנית חדשה'}. אם חסר מידע מהותי, אשאל שאלה קצרה אחת בכל פעם. כתוב “בנה תוכנית אימון” כדי להתחיל.`;
  ...
  const nutritionSetupQuestions: WizardQuestion[] = [
    ...
    { id: 'sourceMode', label: 'איך להתחיל?', type: 'choice', required: true, options: [{ value: 'NEW', label: 'תוכנית חדשה' }, { value: 'LIBRARY', label: 'בחירה מהמאגר', description: 'טעינת תוכנית קיימת לצ׳אט ולעריכה' }] },
    { id: 'templateId', label: 'בחירת תוכנית תזונה מהמאגר', type: 'select', required: true, visibleWhen: answers => answers.sourceMode === 'LIBRARY', options: nutritionTemplatePlans.map(plan => ({ value: plan.id, label: `${plan.title || plan.goal || `תוכנית של ${plan.coachName}`} · ${plan.dailyCalories} קלוריות` })) }
  ];

  const completeNutritionSetup = (answers: WizardAnswers) => {
    startEditingNutrition();
    const source = answers.sourceMode === 'LIBRARY' ? nutritionPlans.find(plan => plan.id === answers.templateId) : undefined;
    setNutritionForm(current => ({
      ...current,
      goal: source?.goal || String(answers.goal || selectedTraineeProfile?.primaryGoal || ''),
      dailyCalories: source?.dailyCalories || Number(answers.dailyCalories || current.dailyCalories),
      proteinGrams: source?.proteinGrams || current.proteinGrams,
      carbsGrams: source?.carbsGrams || current.carbsGrams,
      fatGrams: source?.fatGrams || current.fatGrams,
      hydrationLiters: source?.hydrationLiters || current.hydrationLiters,
      fiberGrams: source?.fiberGrams || current.fiberGrams,
      mealsDescription: source?.mealsDescription || current.mealsDescription,
      categories: source?.categories?.map((meal, index) => ({ ...meal, id: `nutrition-setup-meal-${Date.now()}-${index}` })) || current.categories,
      coachNotes: [source?.coachNotes || current.coachNotes, `מספר ארוחות: ${answers.mealsPerDay || 4}`, `העדפות: ${answers.dietaryPreferences || 'ללא'}`, `מגבלות: ${answers.restrictions || 'ללא'}`].filter(Boolean).join('\n'),
      assistantMessages: [...current.assistantMessages, { id: `nutrition-setup-${Date.now()}`, role: 'ASSISTANT', createdAt: new Date().toISOString(), content: source ? `טענתי את „${source.title || source.goal || 'התוכנית'}” מהמאגר. אפשר לפרסם כפי שהיא או לבקש ממני לערוך אותה.` : 'קיבלתי את שאלון הפתיחה. הנתונים הועברו לצ׳אט; אפשר לכתוב “בנה תוכנית תזונה” ואז לעדכן כל ארוחה דרך הצ׳אט או ישירות.' }]
    }));
  ...
  const navigatorRoutes: WorkoutPlanningRoute[] = [
    'HOME', 'PERSONAL', 'PERSONAL_TRAINEE', 'PERSONAL_GENERAL', 'PERSONAL_SESSION', 'PERSONAL_EXISTING',
  ...
  const openGeneralPersonalBuilder = (programName: string) => {
    ...
    setPersonalBuilderPanel('WORKOUT');
    setPersonalSetupComplete(false);
    setPersonalSetupAnswers({ programName: name });
    setWorkoutPlanningRoute('PERSONAL_BUILDER');
  };
  ...
  const openPersonalLibraryCopy = (plan: WorkoutPlan, targetTraineeId?: string) => {
    ...
    handleUpdateAssistantDraft(draft);
    onUpdateWorkoutAssistantMessages([...workoutAssistantMessages, {
      id: `workout-library-open-${Date.now()}`,
      traineeId: trainee.id,
      coachId: activeUser.id,
      coachName: activeUser.name,
      role: 'ASSISTANT',
  ...
  const openGroupLibraryCopy = (program: GroupWorkoutProgram) => {
    const stamp = Date.now();
    const now = new Date().toISOString();
    const copy: GroupWorkoutProgram = {
      ...program,
      id: `group-library-copy-${stamp}`,
      ...
      stations: (program.stations || []).map((station, stationIndex) => ({
        ...station,
        id: `group-library-copy-station-${stamp}-${stationIndex}`,
        exercises: station.exercises.map((exercise, exerciseIndex) => ({ ...exercise, id: `group-library-copy-station-exercise-${stamp}-${stationIndex}-${exerciseIndex}` }))
      })),
      status: 'DRAFT',
      createdAt: now,
      updatedAt: now,
  ...
  if (guidedWorkoutPlanning && navigatorRoutes.includes(workoutPlanningRoute as WorkoutPlanningRoute)) {
    return <div className="overflow-hidden rounded-xl border border-slate-200 bg-slate-50 shadow-md" id="coach-dashboard">
      ...
      <WorkoutPlanningNavigator
        ...
        onOpenGeneralPersonal={openGeneralPersonalBuilder}
        onOpenPersonalSession={openWorkoutProgramFromCalendar}
        onOpenPersonalPlan={openPersonalLibraryCopy}
            ...
            {activeTab === 'programs' && (
              <div className="space-y-6">
                <div className="flex justify-between items-center border-b border-slate-100 pb-3">
                  <div>
                    <h3 className="text-sm font-bold text-slate-800 flex items-center gap-1.5">
                      {isGeneralPersonalProgram ? `תוכנית אישית כללית: ${generalPersonalTarget?.name}` : `תוכנית אימונים אישית עבור: ${selectedTrainee.name}`}
                    </h3>
                    <p className="text-xs text-slate-500 mt-0.5">
                      {isGeneralPersonalProgram
                        ? 'התוכנית אינה משויכת למתאמן ותישמר במאגר לשימוש עתידי.'
                        : traineeWorkoutPlan
                ...
                {!personalSetupComplete ? <ProgramSetupWizard
                  title={isGeneralPersonalProgram ? `הכנת ${generalPersonalTarget?.name}` : `הכנת תוכנית אישית ל${selectedTrainee.name}`}
                  description={isGeneralPersonalProgram ? 'מגדירים קהל יעד ומבנה כללי, בלי לבחור מתאמן. לאחר מכן הצ׳אט יבנה טיוטה שניתן לערוך ולשמור במאגר.' : 'נאסוף רק את נתוני הפתיחה הנדרשים. לאחר מכן הצ׳אט ישאל רק על מידע שחסר ויפתח לידו טיוטה מלאה לעריכה.'}
                  questions={personalSetupQuestions}
                  initialAnswers={{
                    updateProfile: false,
                    programName: generalPersonalTarget?.name || '',
                ...
                <ProgramBriefPanel title="תקציר התכנון האישי" onEdit={() => setPersonalSetupComplete(false)} items={[
                  { label: isGeneralPersonalProgram ? 'שם התוכנית' : 'מתאמן', value: isGeneralPersonalProgram ? generalPersonalTarget?.name : selectedTrainee.name },
                ...
                {personalSetupAnswers.sourceMode !== 'LIBRARY' && <nav className="personal-builder-menu" aria-label="אזורי בניית תוכנית אישית">
                  ...
                  {!isGeneralPersonalProgram && <button type="button" onClick={() => setPersonalBuilderPanel('PROFILE')} className={personalBuilderPanel === 'PROFILE' ? 'active' : ''}><UserCheck size={18} /><span><strong>נתוני המתאמן</strong><small>מטרות, מגבלות וזיכרון</small></span></button>}
                  ...
                  {!isGeneralPersonalProgram && <button type="button" onClick={() => setPersonalBuilderPanel('SETTINGS')} className={personalBuilderPanel === 'SETTINGS' ? 'active' : ''}><Wrench size={18} /><span><strong>הגדרות</strong><small>זכאות וכלי תכנון</small></span></button>}
                </nav>}
                ...
                {!isGeneralPersonalProgram && personalBuilderPanel === 'PROFILE' && <TraineeMemoryPanel
                  trainee={selectedTrainee}
                  activeUser={activeUser}
                  profile={selectedTraineeProfile}
                ...
                {!isGeneralPersonalProgram && personalBuilderPanel === 'SETTINGS' && (selectedHasWorkoutPlanAccess ? (
                ...
                {!isGeneralPersonalProgram && personalBuilderPanel === 'SETTINGS' && <div className="grid grid-cols-2 gap-3">
            ...
            {activeTab === 'nutrition' && (
              <div className="space-y-6">
                ...
                <ProgramBriefPanel title="תקציר תכנון התזונה" onEdit={() => setNutritionSetupComplete(false)} items={[
                  ...
                  { label: 'מקור', value: nutritionSetupAnswers.sourceMode === 'LIBRARY' ? 'תוכנית מהמאגר' : 'תוכנית חדשה' },
                  { label: 'שמירה במאגר', value: 'אוטומטית בפרסום' }
                ]} />
