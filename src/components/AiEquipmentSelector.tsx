...
export const AiEquipmentSelector: React.FC<AiEquipmentSelectorProps> = ({ activeUser, equipment, selectedIds, onSelectedIdsChange, onUpdateEquipment }) => {
  ...
  return <div className="space-y-3">
    <div className="flex items-center justify-between gap-2"><div><h4 className="text-xs font-black text-white">ציוד שעליו יתבסס האימון</h4><p className="mt-1 text-[9px] text-zinc-400">רק המכשירים המסומנים נשלחים לעוזר.</p></div><button type="button" onClick={() => setShowAdd(value => !value)} className="flex min-h-9 items-center gap-1 rounded-lg bg-amber-400 px-3 text-[10px] font-black text-zinc-950"><Plus size={13} /> הוסף</button></div>
