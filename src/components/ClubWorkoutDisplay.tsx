...
export const ClubWorkoutDisplay: React.FC = () => {
  ...
  return <main className="flex min-h-screen items-center justify-center bg-slate-950 p-8 text-center text-white" dir="rtl">
    <div><RubisLogo size={230} className="mx-auto mb-8" /><MonitorPlay className="mx-auto text-amber-400" size={64} /><h1 className="mt-5 text-4xl font-black">מסך האימון מוכן</h1><p className="mt-3 text-xl text-slate-400">כאשר המאמן יבחר “הצג במסך המועדון”, האימון יופיע כאן אוטומטית.</p>{offline && <p className="mt-4 text-sm font-bold text-rose-400">ממתין לחיבור לשרת התצוגה…</p>}</div>
  </main>;
};
