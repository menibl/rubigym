...
export const CoachTrainingMode: React.FC<CoachTrainingModeProps> = ({ activeUser, users, sessions, onUpdateSessions, workoutPlans, onUpdateWorkoutPlans, groupWorkoutPrograms, onUpdateGroupWorkoutPrograms, onOpenProgram }) => {
  ...
  const [selectedScheduleDate, setSelectedScheduleDate] = useState(() => localDateKey());
  const displayedSessions = useMemo(() => {
    if (scheduleView === 'DAY') {
      return sessions
        .filter(session => session.date === selectedScheduleDate)
        .sort((a, b) => sessionDateTime(a).getTime() - sessionDateTime(b).getTime());
    }
    const anchor = new Date(`${selectedScheduleDate}T12:00:00`);
    ...
    return sessions
      ...
      .sort((a, b) => sessionDateTime(a).getTime() - sessionDateTime(b).getTime());
  }, [scheduleView, selectedScheduleDate, sessions]);
  const shiftSchedule = (direction: -1 | 1) => {
    const date = new Date(`${selectedScheduleDate}T12:00:00`);
    date.setDate(date.getDate() + direction * (scheduleView === 'WEEK' ? 7 : 1));
    setSelectedScheduleDate(localDateKey(date));
  };
  const returnToToday = () => setSelectedScheduleDate(localDateKey());
  ...
  return <section className="space-y-3 p-3 sm:p-5" dir="rtl">
    <div className="rounded-2xl bg-gradient-to-l from-slate-950 to-indigo-950 p-4 text-white">
      ...
      <div className="mt-3 grid grid-cols-[auto_1fr_auto] gap-2">
        <button type="button" onClick={() => shiftSchedule(-1)} className="min-h-11 rounded-xl border border-white/20 bg-white/10 px-3 text-xs font-black text-white" aria-label={scheduleView === 'WEEK' ? 'הצגת השבוע הקודם' : 'הצגת היום הקודם'}>{scheduleView === 'WEEK' ? 'שבוע קודם' : 'יום קודם'}</button>
        <input type="date" value={selectedScheduleDate} onChange={event => setSelectedScheduleDate(event.target.value)} className="min-h-11 w-full rounded-xl border border-white/20 bg-white px-3 text-center font-bold text-slate-900" aria-label="בחירת תאריך, כולל תאריכים ואימונים מהעבר" />
        <button type="button" onClick={() => shiftSchedule(1)} className="min-h-11 rounded-xl border border-white/20 bg-white/10 px-3 text-xs font-black text-white" aria-label={scheduleView === 'WEEK' ? 'הצגת השבוע הבא' : 'הצגת היום הבא'}>{scheduleView === 'WEEK' ? 'שבוע הבא' : 'יום הבא'}</button>
      </div>
      <button type="button" onClick={returnToToday} className="mt-2 min-h-10 w-full rounded-xl bg-amber-300 px-3 text-xs font-black text-slate-950">חזרה להיום</button>
      <p className="mt-2 text-center text-[11px] font-bold text-slate-200">אפשר לבחור כל תאריך כדי לצפות גם באימונים שכבר התקיימו.</p>
    </div>
    {displayedSessions.map(session => {
      const isPastSession = sessionDateTime(session).getTime() < Date.now();
