...
add_table(
    ...
    [
        ...
        ["הצהרת בריאות", "חובה לסמן אישור לפני יצירת החשבון.", "קיים"],
...
callout(
    doc,
    "נקודה לבדיקה שלך",
    "הצהרת בריאות נדרשת בעת יצירת חשבון, אך כרגע אינה נבדקת מחדש בכל לחיצה על הרשמה. יש להחליט האם הצהרה שפג תוקפה או חסרה צריכה לחסום אימונים.",
    "gap",
)
...
add_table(
    ...
    [
        ["קבוצתי חודשי", "כן", "לא כברירת מחדל", "רק כתוספת", "לפי בקשה/תוספת"],
        ["קבוצתי שנתי", "כן", "לא כברירת מחדל", "רק כתוספת", "לפי בקשה/תוספת"],
...
callout(
    doc,
    "פער שדורש חיזוק לפני עלייה לאוויר",
    "בזרימת Open Gym הנוכחית עדיין אין אכיפה מלאה ואחידה של סוג מנוי פתוח, הקפאת מנוי ותשלום משפחתי, כמו באימון קבוצתי. מומלץ לאשר את המפה לעיל ואז להחיל אותה בקוד.",
    "gap",
)
...
add_bullet(doc, "כל משתמש ברשימת המתנה מקבל דירוג עדיפות מתוך פרופיל המשתמש.")
add_bullet(doc, "ביטול מקום של נרשם מקדם אוטומטית את הראשון בתור.")
add_bullet(doc, "לאחר הגעה לסף נקודות שחורות (ברירת מחדל: 3), דירוג העדיפות יורד מ־100 ל־50.")
add_bullet(doc, "נקודות שחורות פגות אוטומטית לאחר חודש, ואז הדירוג מחושב מחדש.")
callout(
    doc,
    "החלטה עסקית נדרשת",
    "יש לאשר אם סדר ההמתנה צריך להיקבע רק לפי דירוג, או לפי שילוב של דירוג + זמן הצטרפות; וכן כיצד מודיעים למשתמש שקודם.",
    "warn",
)
...
add_table(
    ...
    [
        ...
        ["ביטול אימון אישי", "פחות מ־12 שעות לפני האימון.", "נקודה שחורה וחיוב מלא."],
...
callout(
    doc,
    "לפני חיבור סליקה",
    "יש לקבוע מחירים סופיים, מע״מ, תנאי החזר, אמצעי תשלום, חיוב מחזורי, קבלות, כשל חיוב ומדיניות ביטול עסקה.",
    "warn",
)

doc.add_page_break()

heading(doc, "11. החלטות לאישור לפני גרסת Production", 1)
add_checkbox(doc, "גיל מינימלי לפתיחת חשבון עצמאי והאם קטין מחייב אישור הורה.")
add_checkbox(doc, "תוקף הצהרת בריאות: חד־פעמי, שנתי או לפי שינוי רפואי.")
add_checkbox(doc, "האם תאריך תפוגת מנוי חייב להיבדק בזמן הרשמה בנוסף לסטטוס ACTIVE.")
add_checkbox(doc, "האם מותר להירשם לשני אימונים חופפים באותה שעה.")
add_checkbox(doc, "מקסימום אימונים ביום/שבוע לכל מסלול.")
add_checkbox(doc, "כללי Open Gym סופיים: סוגי מנוי מותרים, מכסה חודשית והקפאה.")
add_checkbox(doc, "סדר רשימת המתנה: זמן הצטרפות, עדיפות או שילוב.")
add_checkbox(doc, "ערוץ הודעה בקידום מהמתנה: SMS, WhatsApp, Push או כולם.")
add_checkbox(doc, "חלון ביטול קבוצתי סופי והאם הוא אחיד לכל סוגי האימון.")
add_checkbox(doc, "מדיניות No-show: נקודה, חיוב, חסימת הרשמות או שילוב.")
add_checkbox(doc, "מדיניות משפחות: מי רשאי לשנות/לבטל מנויים של בני משפחה.")
add_checkbox(doc, "מדיניות פרטיות ושמירת מידע רפואי, סיסמאות ונתוני תשלום.")
...
callout(
    doc,
    "המלצת סיום",
    "לאחר אישור סעיף 11, יש לעדכן את כללי Open Gym והצהרת הבריאות, לחבר ספק SMS וספק סליקה, ולהריץ את סעיף 12 כבדיקות קבלה לפני פרסום לציבור.",
    "ok",
)
