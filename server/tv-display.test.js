...
test('rotating TV stations use the responsive station layout without a participant sidebar', async () => {
  ...
  assert.match(script, /function stationLayoutClasses\(layout\)/);
  assert.match(script, /station-cols-/);
  assert.match(script, /station-rows-/);
  ...
  assert.match(styles, /\.tv-stations\.station-cols-3[^}]*--station-width:\s*calc\(33\.333333% - \.766667vw\)/s);
  assert.match(styles, /\.tv-stations\.station-rows-3[^}]*--station-height:\s*calc\(33\.333333% - \.833333vh\)/s);
  assert.doesNotMatch(script, /style="--exercise-columns:/);
